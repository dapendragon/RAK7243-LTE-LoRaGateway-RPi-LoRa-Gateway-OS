...
do_setup_concentrator_shield_rak_rak831() {
    FUN=$(dialog --title "RAK - RAK7243 configuration" --menu "Select configuration:" 15 60 4 \
...
enable_lte(){
    # $1: concentrator type
    # $2: config suffix, eg ".gps"
    FUN=$(dialog --title "Enable LTE auto start" --menu "Enable or not:" 15 60 3 \
        1 "yes" \
        2 "no" \
        3>&1 1>&2 2>&3)
    RET=$?
